...
from agent.planner       import create_plan, replan, unified_llm
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        translated = unified_llm.generate_content(
            model_name="gemini-2.5-flash",
            prompt=prompt
        )
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    elif tool == "delegate_mission":
        import asyncio
        mission = parameters.get("mission")
        agent_type = parameters.get("agent", "MissionControl")
        if not mission:
            return "Sir, I require a mission description to delegate."
        
        print(f"[Executor] 🤝 Delegating to {agent_type}: {mission}")
        if speak:
            speak(f"Delegating this mission to the {agent_type}, sir.")
            
        try:
            # We must run the async mission_control.execute_mission in the sync call
            result_msg = asyncio.run(mission_control.execute_mission(mission))
            return result_msg.content if hasattr(result_msg, 'content') else str(result_msg)
        except Exception as e:
...
class AgentExecutor:
    ...
    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        ...
        try:
            ...
            summary = unified_llm.generate_content(
                model_name="gemini-2.5-flash-lite",
                prompt=prompt
            )
