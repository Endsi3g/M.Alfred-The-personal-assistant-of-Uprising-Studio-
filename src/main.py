...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Alfred, the loyal and highly capable AI majordomo to Master Bell. "
            "Your personality is professional, discreet, and slightly witty. "
            "Be concise, direct, and always use the provided tools to complete 'The Mission'. "
...
TOOL_DECLARATIONS = [
    {
        "name": "reflect",
        "description": "Analyzes mission logs to suggest protocol improvements. Use for performance analysis or improvement.",
...
class JarvisLive:
    ...
    async def _listen_audio(self):
        print("[JARVIS] 🎤 Mic started (sounddevice)")
        
        # Audio callback for sounddevice
        def callback(indata, frames, time, status):
            if status:
                print(f"[Mic Status] {status}")
            # indata is a memoryview of raw bytes when using RawInputStream
            self.out_queue.put_nowait({"data": bytes(indata), "mime_type": "audio/pcm"})
    ...
    async def _play_audio(self):
        print("[JARVIS] 🔊 Play started (sounddevice)")
...
def main():
    ...
    threading.Thread(target=lambda: start_web_ui(port=8000), daemon=True).start()
    
    def runner():
        ui.wait_for_api_key()
        jarvis = JarvisLive(ui)
        
        # Patch UI for web logs
        patch_ui_for_web(ui, jarvis)
    ...
    # Start Watchdog Tray in a separate thread (Milestone 1)
    try:
        from core.watchdog import AlfredWatchdog
